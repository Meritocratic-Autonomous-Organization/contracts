...
*/
contract TokenRequestVesting is AragonApp {
    ...
    */
    ...
    returns (uint256)
    {
        ...
        require(tokenManager.token().balanceOf(tokenManager) >= _requestAmount, ERROR_WRONG_TOKEN_AMOUNT_IN_TM);
    ...
    */
        ...
        auth(FINALISE_TOKEN_REQUEST_ROLE)
    {
        TokenRequest storage tokenRequest = tokenRequests[_tokenRequestId];
        require(tokenRequest.status == Status.Pending, ERROR_NOT_PENDING);
        require(tokenManager.token().balanceOf(tokenManager) >= tokenRequest.requestAmount, ERROR_WRONG_TOKEN_AMOUNT_IN_TM);      
